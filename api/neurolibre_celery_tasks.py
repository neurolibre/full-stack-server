...
def rsync_data(self, comment_id, issue_id, project_name, reviewRepository):
    ...
    try:
        ...
        gh_template_respond("started",task_title,reviewRepository,issue_id,task_id,comment_id, "")
        subprocess.check_call(["rsync", "-avR", remote_path, "/"])
    except subprocess.CalledProcessError as e:
        gh_template_respond("failure",task_title,reviewRepository,issue_id,task_id,comment_id, f"{e.output}")
    ...
    if len(os.listdir(os.path.join("/DATA", project_name))) == 0:
        ...
        gh_template_respond("failure",task_title,reviewRepository,issue_id,task_id,comment_id, f"Data sync was not successful for {project_name}")
    else:
        gh_template_respond("success",task_title,reviewRepository,issue_id,task_id,comment_id, "")
